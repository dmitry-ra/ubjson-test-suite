...
//------------------------------------------------------------------------------
    ...
    ObjectSerializer.prototype.serializeEntity = function(entity) {
        ...
        switch (typeof(entity)) {
            case 'object':
                ...
                break;
            case 'string':
                this.serializeString(entity, true);
                break;
            case 'number':
    ...
    ObjectSerializer.prototype.serializeString = function(string, emitStringType) {
        var utf8value = utf8encode(string);
        var size = utf8value.length;
        if (size == 0) {
            this.addTagItem(Types.EmptyString);
            return;
        }
        if (size == 1) {
            var ch = utf8value.charCodeAt(0);
    ...
    ObjectSerializer.prototype.serializeNumber = function(number, notNull) {
        ...
        if (type != Types.Null) {
            if (type == Types.HighNumber) {
                this.serializeString(number.toString(), false);
            } else {
